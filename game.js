...
/*
  ...
  */
  ...
  ========================= */
  const Storage = (() => {
    const KEY="cloudCatShelterSave_v7";
    const BACKUP_KEY="cloudCatShelterSave_v7_backup";
    const LEGACY=["cloudCatShelterSave_v5","cloudCatShelterSave_v4","cloudCatGameSave_v2","cloudCatGameSave_v1"];
    ...
    function normalize(data){
      ...
      if(Array.isArray(data.cats)){
        ...
        return {version:7,slots,day,actionsLeft:Utils.clamp(Number.isFinite(Number(data.actionsLeft))?Number(data.actionsLeft):4,0,4),coins:Math.max(0,Number(data.coins)||0),houseDamage:Utils.clamp(Number(data.houseDamage)||0),dailyTask:data.dailyTask||null,dailyChoice:data.dailyChoice||null,inventory,activeCatId,cats};
      }
      const day=Math.max(1,Number(data.day)||1);
      const cat=normalizeCat(data,day);
      const inventory={catStrip:Math.max(0,Math.floor(Number(data.inventory?.catStrip)||0)),can:Math.max(0,Math.floor(Number(data.inventory?.can)||0))};
      return {version:7,slots:3,day,actionsLeft:Utils.clamp(Number.isFinite(Number(data.actionsLeft))?Number(data.actionsLeft):4,0,4),coins:Math.max(0,Number(data.coins)||0),houseDamage:Utils.clamp(Number(data.houseDamage)||0),dailyTask:data.dailyTask||null,dailyChoice:data.dailyChoice||null,inventory,activeCatId:cat.id,cats:[cat]};
    }
    function save(data){
  ...
  const Game = (() => {
    ...
    function change(key,value){if(key==="damage"){game.houseDamage=Utils.clamp(game.houseDamage+value);return}if(key==="coins"){game.coins=Math.max(0,game.coins+value);return}game.stats[key]=Utils.clamp(game.stats[key]+value,0,maxFor(key))}
    ...
    function action(actionKey){if(!game||game.actionsLeft<=0)return null;game.actionsLeft--;const hungerBefore=game.stats.hunger;const event=game.breedKey==="americanSilver"&&game.stats.hunger<70&&actionKey==="pet"?{emoji:"🐱",title:"一看见你就开始撒娇",body:`${game.name}一看见你就喵喵叫，还主动抬起头去蹭你的手。它越蹭越起劲，前爪都快离开地面，像是马上就要站起来抱住你。`,effects:{trust:2,intimacy:4},visual:{catState:"happy"}}:EventSystem.getAction(game.personality,actionKey);apply(event.effects);const hungerGain=Math.max(0,game.stats.hunger-hungerBefore);TaskSystem.record(game,{type:actionKey,hungerGain});TaskSystem.refresh(game);game.mood=event.visual?.catState||((event.effects.health<0||event.effects.trust<0)?"angry":actionKey==="play"||actionKey==="feed"?"happy":"neutral");log(event.body);Storage.save(game);return {event,death:isDead()}}
    ...
    function grow(){const old=game.personality;const growthWeights=CONFIG.routes[game.routeKey]?.growth||{spirit:1,demon:1,chaos:1};const next=personalityForBreed(game.breedKey,Utils.roll(growthWeights));game.ageStage="adult";game.personality=next;game.personalityChanged=true;game.mood=next==="chaos"?"angry":"neutral";const story=next==="spirit"?`${game.name}长大后安静了许多，开始每天主动趴在你身边。`:next==="demon"?`你发现家里的杯子总会莫名其妙掉到地上。监控里的${game.name}每次推完都会若无其事地走开。`:`${game.name}静静看着你。下一秒，它突然给了你一爪，随后转身抓坏了新沙发。`;log(`${game.name}成年了，性格从“${CONFIG.personalities[old].name}”定型为“${CONFIG.personalities[next].name}”。`);return {old,next,story}}
  ...
  const UI = (() => {
    ...
    function hudStats(game){
      const items=[
        ['health','💗','健康',game.stats.health,100],['hunger','😊','饱腹',game.stats.hunger,100],['cleanliness','🫧','清洁',game.stats.cleanliness,100],['intimacy','💕','亲密',game.stats.intimacy,Game.maxFor('intimacy')],
    ...
    function renderShelter(){
      ...
      const cards=root.cats.map(cat=>{
        ...
        return `<article class="shelter-cat-card ${active?'active':''}">${active?'<span class="shelter-active-badge">正在照顾</span>':''}<div class="shelter-cat-emoji">${icon}</div><h3 class="shelter-cat-name">${esc(cat.name)}</h3><div class="shelter-cat-meta">${esc(breed.name)} · ${esc(cat.sex)}<br>${cat.ageStage==='adult'?'成年猫':'幼猫'} · ${esc(personality.name)}</div><div class="shelter-mini-stats"><div class="shelter-mini-row"><span>健康</span><div class="bar"><span style="width:${clamp(cat.stats.health)}%"></span></div><b>${Math.round(cat.stats.health)}</b></div><div class="shelter-mini-row"><span>饱腹</span><div class="bar"><span style="width:${clamp(cat.stats.hunger)}%"></span></div><b>${Math.round(cat.stats.hunger)}</b></div></div><button class="shelter-switch-btn" data-shelter-switch="${cat.id}" ${active?'disabled':''}>${active?'当前猫咪':'切换照顾'}</button></article>`;
      });
    ...
    function renderAttributes(){const g=Game.get(),p=CONFIG.personalities[g.personality];$('#attributeDay').textContent=`第${g.day}天`;$('#attributeName').textContent=`${g.name}的档案`;Visual.renderScene('#attributeArt',g);$('#attributeInfo').innerHTML=`<div class="info-box"><small>品种</small><strong>${CONFIG.breeds[g.breedKey].name}</strong></div><div class="info-box"><small>性别</small><strong>${g.sex}</strong></div><div class="info-box"><small>年龄</small><strong>${g.ageStage==='adult'?'成年猫':'幼猫'}</strong></div><div class="info-box"><small>获得方式</small><strong>${g.routeName}</strong></div><div class="info-box"><small>初始性格</small><strong>${CONFIG.personalities[g.initialPersonality].name}</strong></div><div class="info-box"><small>房屋损坏</small><strong>${Math.round(g.houseDamage)}%</strong></div>`;$('#attributeStats').innerHTML=statCards(g,['health','trust','vitality','courage','intimacy','hunger','cleanliness']);$('#attributePersonality').textContent=`当前性格：${p.name}`;$('#attributePersonalityDesc').textContent=p.desc}
  ...
  const App = (() => {
    ...
    function bindGame(){
      $('#adoptBtn').addEventListener('click',async()=>{const result=Game.adopt($('#catNameInput').value.trim());if(result.error){UI.toast(result.error);return}const game=result.game;await prepareGameVisuals(game,'正在准备这只猫的六种状态，完成后互动切换会更流畅。');UI.renderHome();UI.renderShelter();UI.updateContinue();UI.showScreen('homeScreen');startBackgroundAssets(game);setTimeout(()=>{const event=Game.dailyChoiceView();if(event&&!event.resolved)UI.showDailyChoice()},550)});
      ...
      $('#continueBtn').addEventListener('click',async()=>{const saved=Storage.load();Game.set(saved);const game=Game.get();await prepareGameVisuals(game,'正在读取房间和猫咪素材……');UI.renderHome();UI.renderShelter();UI.showScreen('homeScreen');startBackgroundAssets(game);setTimeout(()=>{const event=Game.dailyChoiceView();if(event&&!event.resolved)UI.showDailyChoice()},550)});
